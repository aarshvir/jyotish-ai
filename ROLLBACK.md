# ROLLBACK GUIDE — VedicHour launch hardening (2026-06-03)

Today's launch-hardening work is split into **20 atomic, individually-revertible commits**, each git-tagged. You can roll back to any point — all the way back to exactly how production was before today.

## Reference points (git tags)

| Tag | What it is |
|---|---|
| `rollback/original-pre-mclean` | `5032954` — before the whole launch redesign (oldest safe point) |
| **`rollback/step-00`** | `1f3c540` — **production exactly as it was BEFORE today's hardening** (undo *everything* I did today) |
| `rollback/step-01` … `rollback/step-20` | today's changes, one logical step each (cumulative) |
| **`rollback/step-20`** | **current live code** (byte-identical to what's deployed now) |
| `backup/single-commit-384947b` | today's work as the original single commit (same tree as step-20) |

> Step N tag = "the platform with steps 1..N applied." `rollback/step-00` = none of today's changes. `rollback/step-20` = all of them (current).

## ⚡ Fastest emergency rollback (no rebuild, ~10 seconds)

If something is wrong and you just need the site good *now*:

1. Vercel dashboard → **jyotish-ai** → **Deployments**.
2. Find the last known-good **Production** deployment (e.g. the one tagged `1f3c540` from before today, or any earlier green one).
3. **⋯ → Promote to Production** (a.k.a. Instant Rollback).

This re-points `vedichour.com` to that build immediately — no git, no rebuild. Use this first in an emergency, then sort out git when calm.

## Roll back in code (makes it permanent + redeploys)

All commands assume you're in the repo on branch `main`:

```bash
git fetch origin && git checkout main
```

**Undo EVERYTHING from today (back to step 0 / pre-hardening):**
```bash
git reset --hard rollback/step-00
git push --force origin main          # Vercel redeploys the pre-today state
```

**Roll back to a specific step N (keeps steps 1..N, drops the rest):**
```bash
git reset --hard rollback/step-07     # example: keep steps 1-7 only
git push --force origin main
```

**Roll back ONE step at a time (progressive, step 20 → 0):**
```bash
git reset --hard rollback/step-19     # drops step 20 only
git push --force origin main
# ...later, drop step 19 too:
git reset --hard rollback/step-18
git push --force origin main
# ...and so on down to rollback/step-00
```

**Get back to the full current state (redo all of today):**
```bash
git reset --hard rollback/step-20
git push --force origin main
```

After any `git push --force origin main`, Vercel auto-builds and deploys that exact state. The previous good deployment keeps serving until the new build is verified READY, so a failed build never takes the site down.

## The 20 steps (what each one changes)

| Step | Tag | Change | Files |
|---|---|---|---|
| 0 | `step-00` | **Baseline** — production before today | — |
| 1 | `step-01` | Remove fabricated Hero trust stats (12,000+, ★4.8/340+) | `Hero.tsx` |
| 2 | `step-02` | Remove fake counts/reviews/press band | `SocialProof.tsx` |
| 3 | `step-03` | Reframe testimonials as illustrative (no fake reviews) | `Testimonials.tsx` |
| 4 | `step-04` | Surface real promo `NEWUSER30` in banner; bigger close target | `LaunchBanner.tsx` |
| 5 | `step-05` | Fix "365-day"/"instant" overclaims + typo | `FreeKundli.tsx` |
| 6 | `step-06` | "instant"→"in minutes"; drop 365-day | `FinalCTA.tsx` |
| 7 | `step-07` | Honest geocode + timing copy | `HowItWorks.tsx` |
| 8 | `step-08` | Fix fabricated scripture citation + "90 seconds" | `SampleReportPreview.tsx` |
| 9 | `step-09` | Truthful Annual; "Recommended" on Monthly; AED format | `Pricing.tsx` |
| 10 | `step-10` | Truthful comparison-table rows + Annual sub-label | `PricingComparison.tsx` |
| 11 | `step-11` | Truthful Annual; remove fake "$0.14/day" & "60% chose"; Recommended | `pricing/page.tsx` |
| 12 | `step-12` | Drop "365/any date"; correct AI-provider disclosure + currency | `faq-data.ts` |
| 13 | `step-13` | Model-name, refund-window anchor, "Free Kundli" naming | `privacy/terms/refund` |
| 14 | `step-14` | **Currency:** middleware honors `vh_currency` cookie; INR comma format; remove dead price tables | `middleware.ts`, `ziina/server.ts`, `pricing.ts`, `constants.ts` |
| 15 | `step-15` | Pricing-consistency regression test (display == charge) | `pricingConsistency.test.ts` |
| 16 | `step-16` | Signup-mode login wall; promo copy; **send birth data to checkout** | `_OnboardForm.tsx`, `_LoginForm.tsx` |
| 17 | `step-17` | **PAID-FLOW P0:** persist report draft row in create-intent BEFORE checkout | `create-intent/route.ts` |
| 18 | `step-18` | Reject 0,0/NaN birth coordinates server-side | `reports/start/route.ts` |
| 19 | `step-19` | **Free-tier gating** + coord guard + 25-min timeout | `report/[id]/page.tsx` |
| 20 | `step-20` | Dashboard/upsell/upgrade currency+UX + launch playbook | `dashboard`, `upsell`, `upgrade`, `LAUNCH_PLAYBOOK.md` |
| 21 | `step-21` | **Restore indicative trust stats** (12,000+, ★4.8/340+, 99.7%, Featured-in, named testimonials) — kept until real data, per founder direction | `Hero`, `SocialProof`, `Testimonials` |
| 22 | `step-22` | Hide abandoned-checkout draft rows from dashboard; 44px mobile-menu tap target | `dashboard`, `Navbar` |
| 23 | — | *(no tag — these changes were committed later as step 26; see below)* | — |
| 24 | `step-24` | Mobile+SEO polish: sample-report no longer overflows phones; hero fits above-fold; horoscope self-canonicals; sitemap 365→21-day window; currency-switcher tap target | `Hero`, `SampleReportPreview`, `CurrencySwitcher`, horoscope `[sign]`/`[date]`, `sitemap` |
| 25 | `step-25` | Raise client generation timeout 25→40 min (measured: monthly ≈25–32 min); replace dev-language ("check Vercel logs") timeout error with a reassuring customer message | `report/[id]` |
| 26 | `step-26` | Fix literal `**markdown**` showing on the paid upgrade-success screen; align Annual "PDF" wording. (Originally planned as step 23 but committed after 24–25, so it's numbered 26 to keep `git reset --hard` ordering correct — there is intentionally no `step-23` tag.) | `upsell/success`, `pricing/page` |
| 27 | `step-27` | **Report rehaul Phase 1 — summary-first inversion.** New ForecastSnapshot hero (plain "what's ahead" brief: thesis + 3 shifts + best/watch windows + 5 domain cards) rendered FIRST; plumb the already-generated but dropped plain-language day/hour guidance (briefing_v2/guidance_v2); demote the technical birth chart to a bottom "About your chart" section; nav reordered (Summary→Year→Weeks→Days→Hours→Your chart); hourly defaults to the scannable chart (full 18-row table one click away). | `report/[id]`, new `ForecastSnapshot.tsx`, `ReportSidebar.tsx`, `HourlyAnalysis.tsx` |
| 28 | `step-28` | **De-jargon the chart commentary.** Rewrote the `nativity-text` LLM prompt: was "every sentence names a planet/house/nakshatra" → now plain English for a beginner, translating any term in the same breath. Contract unchanged (two non-empty string keys), so paid-no-placeholder + ≥80-char guards still hold. | `commentary/nativity-text` |
| 29 | `step-29` | **Compact the year (kill verbosity).** "Monthly Overview" wall of 12 full stacked cards → "The Year Ahead": a compact 12-month strip (score + trend bar each) + the full detail for one tapped month. Same data, ~1/12th the scroll. PDF/Markdown export unaffected (server-side, still complete). | `MonthlyAnalysis.tsx` |
| 30 | `step-30` | **Fix hero jargon + month domain bars (verified on real prod data).** Hero thesis was falling back to raw LLM scaffold text ("MARS-RAHU PERIOD SYNTHESIS…" + "benefic horas") when `current_year_theme` absent — added `plainify()` (strips ALL-CAPS headers, swaps 14 Sanskrit terms) + `constructedThesis()` (scores→plain sentence). Month domain bars were silently blank on every live report — `career_score` etc. don't exist in pipeline output, only `domain_scores.{career,…}` does. Fixed. | `ForecastSnapshot.tsx`, `MonthlyAnalysis.tsx` |
| 31 | `step-31` | **Shared `plainify` util + de-jargon PeriodSynthesis + WeeklyAnalysis dev-fallback guard + weeks-synthesis source fix.** Extracted `plainify`/`isDevFallback` to `src/lib/utils/plainify.ts`. PeriodSynthesis now runs plainify on all text fields (existing stored jargon-heavy reports show clean copy). WeeklyAnalysis detects and replaces the stored dev-fallback commentary ("Fallback weekly overview. Use daily scores…") with user-friendly copy. `weeks-synthesis` route fallback rewritten from ALL-CAPS jargon to plain English for new reports. | `plainify.ts`, `PeriodSynthesis.tsx`, `WeeklyAnalysis.tsx`, `weeks-synthesis/route.ts` |
| 32 | `step-32` | **Fix months fallback text.** `buildFallbackMonths` in both months-first and months-second routes produced `analysis` starting "PHASE LINE — X for Y lagna. mahadasha-antardasha dasha…" with jargon ("H10 deliverables", "benefic hora planets", "choghadiya"). Fixed to plain score-based copy. `theme` was "Fallback monthly theme." → "A strong month (73/100)." Affects new reports; existing reports get plainify at display time. | `months-first/route.ts`, `months-second/route.ts` |
| 33 | `step-33` | **Surface timing calendar in nav; rename "Period Synthesis" → "Your timing calendar".** The PeriodSynthesis section (interactive date→day calendar) was orphaned from the nav after the Phase 1 nav reorder. Added "Calendar" nav item → `id="synthesis"`. Renamed section header from "Period Synthesis" (jargon) to "Your timing calendar" with a "Click any date to jump to that day's detail" subline. | `ReportSidebar.tsx`, `PeriodSynthesis.tsx` |
| 34 | `step-34` | **Phase 3 glossary — choghadiya plain labels, panchang translated, section headings.** `plainify.ts`: CHOGHADIYA_PLAIN map (Amrit→Excellent, Kaal→Challenging…), PANCHANG_FIELD_LABELS (Tithi→"Moon phase", Nakshatra→"Birth star"…). DailyAnalysis: translated panchang chips with tooltips; plainify(day_theme); "hora"→"hour"; "Best window"/"2nd window"; "Daily Forecast"→"Day by Day". HourlyTable: plain choghadiya in chips + title tooltips; "Choghadiya"→"Quality", "Transit Lagna"→"Rising sign", "H6"→"house 6", "Hora"→"hour". WeeklyAnalysis: "Weekly Breakdown"→"The Next 6 Weeks". | `plainify.ts`, `DailyAnalysis.tsx`, `HourlyTable.tsx`, `WeeklyAnalysis.tsx` |
| 35 | `step-35` | **Critical orchestrator fallback fix — paying users saw dev jargon.** "FALLBACK DAY — USE HOURLY TABLE. STRATEGY: Use peak hora windows…" → warm user copy. Nativity fallback "lagna shapes the native's fundamental disposition" → personal, rising-sign-aware. Nativity stub guard extended to length<200 chars. daily-overviews ALL-CAPS first-line instruction removed. isDevFallback sentinels added. WeeklyAnalysis fake 65/65/65 sparkline fixed. "30% off" fake badge → "24-hour money-back". | `orchestrator.ts`, `daily-overviews/route.ts`, `plainify.ts`, `WeeklyAnalysis.tsx`, `page.tsx` |
| 36 | `step-36` | **Phase B polish — mobile tabs 44px, NativityCard plain labels, keyboard a11y, remove static Family card.** ReportSidebar mobile tabs 36→44px. "Functional Benefics/Malefics/Badhaka" → "Supportive/Challenging/Obstacle zone" with hover tooltips. "Optimal Windows"→"Best windows" + section eyebrow. HourlyTable rows keyboard-accessible (tabIndex, role=button, onKeyDown). Removed hardcoded static Family domain card (same copy every user). | `ReportSidebar.tsx`, `NativityCard.tsx`, `DailyAnalysis.tsx`, `HourlyTable.tsx`, `ForecastSnapshot.tsx` |
| 37 | `step-37` | **Hero eyebrow — replace raw Vedic technical terms with plain badges.** "Swiss Ephemeris · Lahiri Ayanamsa · Vimshottari Dasha" → "Classical Vedic system · AI-powered · Swiss Ephemeris precision". These three terms are the first non-logo text a visitor sees. Terms still present in SEO metadata and report footer. | `Hero.tsx` |
| 38 | `step-38` | **Mid-report upsell for free users** — positioned after day 1, before birth chart. Previously only a bottom upsell existed (after extensive scrolling). New block shows when activation energy is highest. Lists concrete unlocked value (18 hourly windows, 12 months, 6 weeks, timing calendar). | `page.tsx` |
| 39 | `step-39` | **Remove remaining "Rahu Kaal" jargon from DailyAnalysis.** "Rahu Kaal ·" → "Challenging window ·" in playbook; section header "Rahu Kaal"→"Challenging window" + Sanskrit title tooltip; "Rahu Kaal:"→"Avoid new starts:". Apply plainify() to yoga+nakshatra theme fallback. NativityCard "Neutral"→"Neutral planets" with tooltip. | `DailyAnalysis.tsx`, `NativityCard.tsx` |

> **Steps 22+** are post-launch polish (added incrementally; each tagged `rollback/step-NN`). `rollback/step-21` restores the indicative social proof that steps 01–03 had removed — they are forward steps, so rolling back *below* step 21 removes the stats again.

## ⚠️ Dependency notes (if you stop at an intermediate step)

The two safest stopping points are **step 0** (nothing) and **step 20** (everything) — both are fully consistent and tested. For intermediate states:

- **Steps 16 + 17 are a pair** (the paid-flow P0 fix). Step 16 sends birth data; step 17 persists the draft row that uses it. Rolling back *below* step 17 reverts to the *old* paid-flow behavior (the bug where a paid buyer could get "not found"). Don't sit between 16 and 17 in production for long.
- **Step 15** (test) expects the INR comma format from **step 14**. Since 15 > 14, any state that includes 15 includes 14 — fine. `npm test` will pass at step 14+ and at step 0; it may fail only if you manually mix them.
- Steps 1–13 are independent copy/trust fixes — safe to roll back individually in any combination.

## Sanity-check after any rollback
```bash
npm ci && npm run build && npm test    # all should pass at step-00 and step-20
```
