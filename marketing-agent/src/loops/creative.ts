...
THE SPINE — ${SHOTS_MIN} to ${SHOTS_MAX} shots, ${REEL_SEC_MIN}-${REEL_SEC_MAX} seconds total:
  ...
  2. presenter, ~4s — THE SECOND HALF OF THE THOUGHT, and the beat that turns your cold open into a QUESTION. He is still talking, no cut away. By the end of this line the viewer should want to know WHEN.
     WHY THIS BEAT EXISTS, because it was not here before and the evidence for adding it is the most one-sided this engine has produced. The product used to be nailed to shot 2, and the viewer-reviewer then killed EIGHTEEN CONSECUTIVE VARIANTS at the 3-second mark, in these words: "unexplained product grid interrupts the awkward brunch story before it develops", "the product screen arrives before I hear what Chachi actually said", "I want the spoon explanation, but get an unrelated product grid", "the vulnerable agreement setup is replaced by a generic glowing grid", "a funny misunderstanding gets interrupted by an unearned product screen". Not one of those was a writing problem. Every one of them was the SAME structural problem: a commercial starting three seconds into a story, before the story had asked anything.
     So the product now waits one beat, and that beat's whole job is to make the screen wanted. The rule is: THE PRODUCT ARRIVES THE MOMENT THE VIEWER WANTS IT, NEVER BEFORE.
  3. screencap, at most ${FIRST_PRODUCT_MAX_SEC}s — THE INSERT. A glance: the real report, hour slots down the day, clearer windows lit, heavier ones dim. NO WORDS OVER IT. It is the answer to the question shot 2 just raised, which is exactly why it no longer feels like an ad pivot.
     YOU MAY PULL THIS FORWARD TO SHOT 2 — but only if your cold open ALREADY asks something a screen can answer, all by itself, to a stranger with zero context. "Aaj girlfriend ka naam lunga" qualifies: the viewer instantly wants to know when, and how it goes. Almost nothing else does. If you are not certain, keep the insert at shot 3 — that is the default for a reason, and the reason is eighteen rejections.
     Either way the product is on screen by second ${PROOF_STARTS_BY_SEC}, and no b-roll may stand in front of it. The only thing allowed to delay the product is the presenter earning it.
  4. presenter, up to ${LONG_BEAT_MAX_SEC}s — the ONE long beat in the reel: the real, specific thing that actually happened. Only one shot may be this long. This is where the remembered detail goes, in full, with its specifics intact.
...
[{"hookText":"...","spokenScript":"...","shotList":[{"kind":"presenter","seconds":3,"visualPrompt":"...","dialogue":"<cold open: one complete sentence a stranger understands, max 6 words>"},{"kind":"presenter","seconds":4,"visualPrompt":"<same man, same place, DIFFERENT shot size>","dialogue":"<the beat that turns the cold open into a question — still him, do NOT cut to product yet>"},{"kind":"screencap","seconds":2,"visualPrompt":"...","narration":""},{"kind":"presenter","seconds":6,"visualPrompt":"...","dialogue":"<the remembered detail, in full, with its specifics intact>"},{"kind":"screencap","seconds":4,"visualPrompt":"...","narration":""},{"kind":"presenter","seconds":4,"visualPrompt":"...","dialogue":"<lands on the cold open, and says vedichour.com>"}],"onScreenCaptions":["..."],"cta":"...","hashtags":["#..."],"youtubeTitle":"...","youtubeDescription":"...","language":"hinglish","hookFamily":"${idea.tags.hookFamily}","decisionDomain":"${idea.tags.decisionDomain}","emotionalRegister":"${idea.tags.emotionalRegister}","durationTargetSec":${idea.tags.durationTargetSec}}]`;
}
