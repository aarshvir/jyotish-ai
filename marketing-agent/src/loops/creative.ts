...
THE SPINE — ${SHOTS_MIN} to ${SHOTS_MAX} shots, ${REEL_SEC_MIN}-${REEL_SEC_MAX} seconds total:
  ...
  3. screencap, at most ${FIRST_PRODUCT_MAX_SEC}s — THE INSERT. A glance, no words over it, arriving as the answer to what he just said.
     AND IT MUST BE LEGIBLE IN TWO SECONDS, which is why it is no longer the scrolling grid. The reviewer's word for the old insert, on four separate reels, was "UNEXPLAINED": "unexplained product grid", "another contextless grid", "an unrelated product grid". He was right and it was not a taste problem — two seconds of a moving 18-row list is genuinely unreadable, so it registers as a flash of generic app footage rather than as proof of anything. Choose a capture a stranger can PARSE at a glance: two hours for the same task, side by side, one clearly lighter and one clearly heavier. The scrolling grid is a fine choice for the HOLD, where it has time; it has never once worked as the insert.
...
[{"hookText":"...","spokenScript":"...","shotList":[{"kind":"presenter","seconds":3,"visualPrompt":"...","dialogue":"<cold open: one complete sentence a stranger understands, max 6 words>"},{"kind":"presenter","seconds":5,"visualPrompt":"<same man, same place, DIFFERENT shot size>","dialogue":"<THE REMEMBERED DETAIL, in full, specifics intact — the best line in the reel, and it comes BEFORE any product screen>"},{"kind":"screencap","seconds":2,"visualPrompt":"<two hours for the same task side by side, one lighter one heavier — legible in 2s, NOT the scrolling grid>","narration":""},{"kind":"presenter","seconds":4,"visualPrompt":"...","dialogue":"<the turn — same moment, but now he knows when>"},{"kind":"screencap","seconds":4,"visualPrompt":"...","narration":""},{"kind":"presenter","seconds":4,"visualPrompt":"...","dialogue":"<lands on the cold open, and says vedichour.com>"}],"onScreenCaptions":["..."],"cta":"...","hashtags":["#..."],"youtubeTitle":"...","youtubeDescription":"...","language":"hinglish","hookFamily":"${idea.tags.hookFamily}","decisionDomain":"${idea.tags.decisionDomain}","emotionalRegister":"${idea.tags.emotionalRegister}","durationTargetSec":${idea.tags.durationTargetSec}}]`;
}
