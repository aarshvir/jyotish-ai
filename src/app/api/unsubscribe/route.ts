 ...
 *   - GET (a human clicking the link): verify token → suppress → friendly HTML page.
...
export async function GET(req: NextRequest) {
  ...
  const ok = await suppress(token);
  const body = ok
    ? { title: 'You&rsquo;re unsubscribed', msg: 'You won&rsquo;t receive any more marketing emails from VedicHour. Your account and any reports are untouched.' }
...
a{display:inline-block;color:#0a0a1a;background:#d4af37;text-decoration:none;font-weight:600;padding:11px 22px;border-radius:9px;font-size:14px}</style></head>
<body><div class="card"><h1>${body.title}</h1><p>${body.msg}</p><a href="https://www.vedichour.com">Back to VedicHour</a></div></body></html>`;
