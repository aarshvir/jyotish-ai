...
${codeLine ? `${codeLine}\n` : ''}${logText ? `\n--- pipeline log ---\n${logText}` : ''}`.trim();
  ...
  const createReportRecord = useCallback(async () => {
    ...
    const paymentStatusForInsert =
      cached?.payment_status ??
      (params.get('bypass') ? 'bypass' : (planType === 'preview' ? 'free' : 'unpaid'));
    ...
    const { error } = await supabase.from('reports').insert({
      ...
      plan_type: planType,
      status: 'generating',
      payment_status: paymentStatusForInsert,
    });
  ...
  const kickOffBackgroundGeneration = useCallback(async (opts?: { forceRestart?: boolean }) => {
    ...
    const startBody = {
      ...
      payment_status: paymentStatusForStart,
