...
// Map raw Supabase/OAuth errors to friendly copy.
...
function LoginInner() {
  ...
  useEffect(() => {
    ...
    // Surface OAuth errors redirected back via ?error= / ?error_description=
      ...
      <div className="w-full max-w-[420px] bg-white/[0.03] border border-amber/15 rounded-2xl p-8 sm:p-10">
        ...
        <form onSubmit={(e) => void handleEmailAuth(e)} noValidate>
          <div className="mb-4">
            <label
              htmlFor="login-email"
              className="block font-body text-xs font-medium text-dust mb-1.5"
            >
              Email
            </label>
            <input
          ...
          <div className="mb-2">
            <div className="flex items-center justify-between mb-1.5">
              <label
                ...
                className="block font-body text-xs font-medium text-dust"
              >
                Password
              </label>
              {mode === 'login' && (
          ...
          {error && (
            <div
              role="alert"
              aria-live="polite"
              className="px-4 py-2.5 mt-4 mb-1 bg-caution/10 border border-caution/30 rounded-card font-body text-body-sm text-caution/90"
            >
              {error}
            </div>
          )}
          {info && !error && (
            <div
              role="status"
              aria-live="polite"
              className="px-4 py-2.5 mt-4 mb-1 bg-success/10 border border-success/30 rounded-card font-body text-body-sm text-success"
            >
              {info}
            </div>
          )}
