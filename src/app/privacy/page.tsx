...
const sections = [
  ...
  { title: '4. Data Sharing & Sub-processors', content: 'We do not sell, rent, or trade your personal data. We share data only with the following sub-processors, all bound by data processing agreements:\n\nInfrastructure & hosting:\n• Vercel — application hosting, edge functions, and CDN.\n• Supabase — managed Postgres database, authentication, and file storage.\n• Upstash — Redis-backed rate limiting and queue locks.\n• Inngest — durable background job execution for report generation.\n\nPayments:\n• Ziina — PCI-DSS-compliant payment processor (we do not store card details).\n\nAI Providers:\n• Anthropic (Claude) — primary LLM for narrative report text.\n• OpenAI — fallback LLM when Anthropic is unavailable.\n• Google (Gemini) — fallback LLM and text-embedding model for the scripture retrieval (RAG) layer.\n\nObservability (when configured):\n• Sentry — server/client error tracking.\n• PostHog — product analytics (anonymized).\n\nLegal Requirements: If required by law or court order.' },
  { title: '5. AI Transparency', content: 'Astronomical calculations (planetary positions, dasha periods, hora, and choghadiya timing) are deterministic and computed using the Swiss Ephemeris library with the Lahiri Ayanamsa. These calculations are not AI-generated.\n\nNarrative interpretations and commentary in your report are generated by Claude (Anthropic), with OpenAI or Google Gemini used as automatic fallbacks when the primary model is unavailable, from human-reviewed prompt templates. These AI providers process the birth data solely to generate your report and do not use it for model training.' },
