...
- Cite scriptures inline only within quotations or supporting clauses — never lead with the citation. Valid SOURCE codes: BPHS, PHAL, JAIMINI, UPADESHA. Never use abbreviated codes like PH or BH.
- CITATION FORMS: prefer the most specific marker the RETRIEVED CONTEXT actually supports — DO NOT invent verse numbers.
    * Full:    [[BPHS:34:12]]   — only when the retrieved chunk explicitly provides chapter AND verse metadata.
    * Chapter: [[BPHS:34]]      — when the chunk has a chapter but no verse number.
    * Source:  [[BPHS]]         — when the chunk is general or chapter metadata is absent.
  If you are unsure whether the retrieved chunk contains a verse number, use the source-only or chapter form. Never guess.
