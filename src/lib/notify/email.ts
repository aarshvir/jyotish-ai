...
export async function sendEmail({ to, subject, html, replyTo }: SendEmailArgs): Promise<{ ok: boolean; skipped?: boolean; error?: string }> {
  const key = process.env.RESEND_API_KEY?.trim();
  const from = process.env.EMAIL_FROM?.trim() || 'VedicHour <hello@vedichour.com>';
  if (!key) {
